...
function DishDetailPage() {
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      ...
      {dish.category && <p>類型:{dish.category.join(', ')}</p>}
      {dish.ingredients && <p>食材:{dish.ingredients.join('、')}</p>}
      ...
      {dish.hasRecipe && dish.recipe && (
        <div style={{ marginTop: 20, borderTop: '1px solid #ccc', paddingTop: 16 }}>
          <h2>食譜內容</h2>
          {dish.recipe.coverPhotoUrl && (
            <img src={dish.recipe.coverPhotoUrl} alt={dish.name} style={{ maxWidth: '100%' }} />
          )}
          {dish.recipe.content?.map((block, i) =>
            block.type === 'text' ? (
              <p key={i}>{block.text}</p>
            ) : (
              <img key={i} src={block.url} alt="" style={{ maxWidth: '100%', margin: '8px 0' }} />
            )
          )}
        </div>
      )}
